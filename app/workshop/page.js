...
export default function WorkshopPage() {
  ...
  const addQuest = (newQuest, cost) => {
    ...
    const questCoins = coinsReward
      ? ((r) => r.amount ?? r.data?.amount ?? 0)(coinsReward)
      : 0;
    const experienceReward = newQuest.rewards.find(
      (r) => r.type === "experience"
    );
    const questExperience = experienceReward
      ? ((r) => r.amount ?? r.data?.amount ?? 0)(experienceReward)
      : 0;
    ...
    newQuest.sub_quests = newQuest.sub_quests.map((sub) => {
      ...
      if (questCoins > 0) {
        subRewards.push(
          new Reward("coins", { amount: Math.floor(questCoins * 0.05) })
        );
      }
      if (questExperience > 0) {
        subRewards.push(
          new Reward("experience", {
            amount: Math.floor(questExperience * 0.05),
          })
        );
      }
